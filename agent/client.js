...
class AgentClient {
    ...
    async registerAgent() {
        ...
        try {
            ...
            const { agentId, actionsCollectionPath } = (await response.json());
        ...
        try {
            ...
            setInterval(async () => {
                try {
                    ...
                    console.error('Error in fallback polling:', err.message);
                }
            }, 30000);
        }
    }
    async subscribeToResource(sessionId, resourceUri) {
        if (!sessionId)
            return;
        try {
            ...
            console.error('Error subscribing to resource:', err.message);
        }
    }
    async fetchAndProcessActions() {
        if (!this.agentId)
            return;
        try {
            ...
            for (const action of actions) {
                ...
                if (task === 'runModule') {
                    ...
                    try {
                        ...
                        await this.updateActionStatus(actionId, 'error', err.message);
                    }
                }
                else {
                    if (this.debug) {
            ...
            console.error('Error fetching/processing actions:', err.message);
        }
    }
    async updateActionStatus(actionId, status, error) {
        if (!this.agentId)
            return;
        try {
            const response = await fetch(`${this.serverUrl}/`, {
            ...
            console.error('Error updating action status:', err.message);
        }
    }
}
