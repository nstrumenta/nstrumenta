...
const run = new Promise(async (resolve, reject) => {
  ...
  try {
    const { status } = await nstrumenta.connect({ wsUrl: process.env.NSTRUMENTA_WS_URL || 'ws://localhost:8088' });
    console.log(`connected ${ClientStatus[status]}`);
    // see that we don't crash when sending a message
    console.log('sending test message');
    nstrumenta.send('#test', { test: `[${testId}] test output` });
