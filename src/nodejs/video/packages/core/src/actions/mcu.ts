...
export const listenMixedAudio =
  (room: Room) => async (subscriberId: string, infos: MediaInfo[]) => {
...
export const addMixedAudioTrack = (room: Room) => async (mixerId: string, info: MediaInfo) => {
  const media = room.medias[info.mediaId];
  const mcu = room.getMCU(mixerId);
  mcu.inputMedia(media);
};

export const removeMixedAudioTrack = (room: Room) => async (mixerId: string, info: MediaInfo) => {
  const media = room.medias[info.mediaId];
  const mcu = room.getMCU(mixerId);
